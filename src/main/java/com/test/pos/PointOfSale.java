...
public class PointOfSale implements POS {
	...
	public PointOfSale(final Display display, final Printer printer,
			final Catalog<Product> catalog, final TaxApplier taxApplier,
			final Sale sale) {
	...
	private void addEntryToPrinter(final Product product) {
		printer.addEntry(new ReportEntry(product.barcode(), product.netPrice(),
				product.apply(taxApplier)));
	}
